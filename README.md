...
- 寬度
- 高度
每個參數接受一個數字，該數字定義預覽視窗中顯示的影像大小（以像素為單位）。
對於 rpicam-still、rpicam-jpeg 和 rpicam-vid，請指定輸出解析度。
對於 rpicam-raw，請指定原始幀解析度。對於具有 2×2 bin 讀出模式的相機，請指定等於或小於 bin 模式的分辨率，以捕獲 2×2 bin 原始幀。
對於 rpicam-hello，沒有效果。
錄製 1080p 影片
...
以 2028×1520 的分辨率擷取 JPEG 檔案。如果使用 HQ 相機，則使用 2×2 bin 模式，因此原始檔案 (test.ng) 包含 2028×1520 的原始拜耳影像。
```bibtex
rpicam-still -r -o test.jpg --width 2028 --height 1520
```
--viewfinder寬度
--取景器高度
每個參數接受一個數字，該數字定義預覽視窗中顯示的影像大小（以像素為單位）。預覽視窗的大小不受影響，因為影像會調整大小以適應預覽視窗。捕獲的靜態影像或視訊不受影響。
```bibtex
rpicam-still --viewfinder-width 1920 --viewfinder-height 1080
```

--mode 允許以下列冒號分隔的格式指定相機模式：<寬度>:<高度>:<位元深度>:<packing>。如果提供的值不完全匹配，系統將選擇與感測器最接近的可用選項。您可以使用 packed(P) 或 unpacked(U) 打包格式，這會影響儲存的影片和靜態影像的格式，但不會影響傳輸到預覽視窗的幀的格式。
位深 (Bit-depth) 和打包 (Packing) 是可選的。預設情況下，位元深 (Bit-depth) 為 12，打包 (Packing) 設定為 P（打包）。
有關感測器可用的位深、解析度和打包選項的信息，請參閱 list-cameras。
如下圖所示：

4056:3040:12:P - 4056×3040 分辨率，12 位元/像素，打包。
1632:1224:10 - 1632×1224 分辨率，10 位元/像素。
2592:1944:10:U - 2592×1944 分辨率，10 位元/像素，未打包。
3264：2448——3264×2448解析度。
--viewfinder-mode 與 mode 選項相同，但它適用於傳遞給預覽視窗的資料。有關更多信息，請參閱模式文件。
--lores-width 和 --lores-height
提供來自相機的第二條低解析度影像串流，並按比例縮小至指定尺寸。每個參數接受一個數字來定義低解析度影像流的尺寸（以像素為單位）。預覽和視訊模式下均可使用。不提供靜態捕獲功能。對於 RPICAM-vid，請停用額外的色彩去噪處理。此功能適用於與影像後處理結合的影像分析。
```bibtex
...
--rotation 旋轉從感測器提取的影像。僅接受 0 或 180 度的值。

```bibtex
rpicam-hello --rotation 0
```
--roi 裁剪從整個感測器域提取的影像。接受四個十進制值，範圍從 0 到 1，格式如下：<x>,<y>,<w>,<h>。每個值都以 0 到 1 之間的小數表示可用寬度和高度的百分比。
這些值定義以下比例：
<x>：提取圖像前跳過的 X 座標
<y>：提取圖像前跳過的 Y 座標
<w>：要提取的圖像寬度
<h>：要提取的圖像高度
預設值為 0,0,1,1（從第一個 X 座標和第一個 Y 座標開始，使用 100% 的圖像寬度和 100% 的圖像寬度）。
範例：
rpicam-hello --roi 0.25, 0.25, 0.5, 0.5 選擇從影像中心裁切的總像素數的一半（跳過前 25% 的 X 座標和前 25% 的 Y 座標，使用影像總寬度的 50% 和影像總高度的 50%）。
rpicam-hello --roi 0,0,0.25,0.25 選擇從影像左上角裁切的總像素數的四分之一（跳過 X 座標的第一個 0% 和 Y 座標的第一個 0%，使用影像寬度的 25% 和影像高度的 25%）。

--HDR 預設：關閉，以 HDR 模式運作相機。如果未傳遞任何值，則假定為自動。接受以下值之一：
off－禁用 HDR。
自動 - 在支援的裝置上啟用 HDR。如果感測器內建 HDR 模式可用，則使用感測器內建的 HDR 模式。如果感測器沒有內建 HDR 模式，則使用板載 HDR 模式（如果可用）。
single-exp 可在支援的設備上啟用 HDR。如果可用，請使用感測器內建的 HDR 模式。如果感測器沒有內建 HDR 模式，則使用板載 HDR 模式（如果可用）。

```bibtex
rpicam-hello——hdr
```
即使感測器內建了 HDR 模式，也請使用板載 HDR 模式（如果可用）。如果板載 HDR 模式不可用，HDR 將被停用。 Raspberry
Pi 5 及更高版本的裝置均已內建 HDR 模式。
若要檢查感測器內建的 HDR 模式，請將此選項新增至相機清單。
## 相機控制選項

以下選項控制影響相機影像品質的影像處理和演算法。

鋒利
設定影像清晰度。接受以下範圍內的值：

0.0 表示不應用銳化
大於 0.0 但小於 1.0 的值應用的銳利化程度低於預設值
1.0 應用預設銳化量
大於 1.0 的值會施加額外的銳化
```bibtex
...
```
亮度
指定影像亮度，該亮度將作為輸出影像中所有像素的偏移量添加。可接受以下範圍內的值：
...
```
電動車
指定影像的曝光值補償。接受一個數值，該數值將沿著以下頻譜傳遞到自動曝光/增益控制 (AEC/AGC) 處理演算法的目標值：
...
```
快門
指定使用快門的曝光時間，以微秒為單位。使用此選項時，增益仍然可以變化。如果相機的幀速率過高，導致無法達到指定的曝光時間（例如，幀速率為 1 fps，曝光時間為 10,000 微秒），則感測器將使用幀速率允許的最長曝光時間。
有關官方相機的最小和最大快門時間列表。高於最大值的值將導致未定義的行為。
```bibtex
rpicam-hello --shutter 10000
```
獲得
analoggain 的效果與 gain 相同
設定組合的類比和數位增益。當感測器驅動器可以提供所需的增益時，僅使用類比增益。當類比增益達到最大值時，ISP 將應用數位增益。接受數值。
有關官方相機的類比增益限制列表，請參閱相機硬體文件。
有時，即使未超過類比增益限制，數位增益也可能超過 1.0。這可能發生在以下情況：
任何一個顏色增益都降至 1.0 以下，這將導致數位增益穩定在 1.0/min(red_gain,blue_gain)。這使應用於任何顏色通道的總數位增益保持在 1.0 以上，以避免變色偽影。
自動曝光/增益控制 (AEC/AGC) 變化期間略有變化。

```bibtex
rpicam-hello --gain 0.8
```
測光預設值：中心
設定自動曝光/增益控制 (AEC/AGC) 演算法的測光模式。接受以下值：

中央重點測光
點測光
平均 - 平均或整幀測光
custom - 相機調整檔案中定義的自訂測光模式
有關定義自訂測光模式以及調整現有測光模式中的區域權重的更多信息。
```bibtex
rpicam-hello --exposure sport
```
awb
設定曝光設定檔。更改曝光設定檔不會影響影像曝光。相反，不同的模式會調整增益設定以達到相同的最終效果。接受以下值：可用的白平衡模式：
...
這些數值僅為近似值：實際值可能會根據相機調校而有所不同。
沒有哪種模式會完全停用 AWB。您可以使用awbgains來調整色彩增益。
有關 AWB 模式的更多信息，包括如何定義自訂模式，請參閱Raspberry Pi 相機和 libcamera 的調校指南。
